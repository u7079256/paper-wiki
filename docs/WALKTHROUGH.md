...
### 背景
小林是二年级博士生，导师说"看看 3DGS 和 diffusion 做 avatar 的可能性"。
她读过 3-4 篇论文，对领域没有系统认知。
...
> 如果小林之前填了 scope fence 把 NeRF 排除，第 7 篇会标 `[FENCE]` 且不在推荐列表里。

小林选了前 5 篇导入，跑了两轮 `/wiki-compile`。wiki 长到 8 篇论文、3 个概念、1 个 gap。
...
wiki 有 12 篇论文、5 个概念、2 个 gap。小林开始觉得有些方向可以做。
...
场景做了类似的事——你需要限定到 avatar-specific task"。小林修正 gap 描述。
...
Teach 输出分组的论文列表，每篇一句话定位。小林用这个作为 related work 的骨架。
...
### 背景
王博士是博后，已经在 avatar 领域发了 3 篇论文。他有一个明确想法：
...
**`/wiki-init`** — 王博士不需要留空 scope fence，他很清楚边界：
...
王博士在 gap frontmatter 里设 `novelty_verified: true`，心里有底了。
...
### 背景
陈博士 6 个月前建了一个 avatar animation wiki（50 篇论文、8 个概念、4 个 gap），
...
小林用 gap-focused 模式 `/wiki-ideate wiki/gaps/hand-avatar-contact.md`，
